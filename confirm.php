...
foreach($_GET as $key => $value)
{
    if ($key == 'custName') {
        ...
        $message .= '<li>Last Name: ' . $value . '</li>';
        echo '<li>Last Name: ' . $value . '</li>';
        $lastName = $value;
    } else if ($key == 'phone') {
...
if (mysqli_query($conn, $sql)) {
    ...
    $grabNum = "select * from res_info";
    $result = @mysqli_query($conn, $grabNum);
    $row = mysqli_fetch_assoc($result);
    $orderNum = $row['order_num'];

    $insertCustomer = "insert into cust_info (first_name, last_name, email, phone, order_num)
            values ('".$firstName."', '".$lastName."' , '".$email."', '".$phone."' , '".$orderNum."')";
